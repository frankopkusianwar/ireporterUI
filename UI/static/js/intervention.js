        ...
        .then(function (message){
            if(message['data']){
                ...
                for (item = message['data'].length-1; item >= 0; item--){
                    ...
                    if(red_status ==='draft'){
	                    records+=`<div class="redflag-content"><item1><img src="static/images/intervention2.jpg"><span class="location" id="edit_txt">
                                </br></br><textarea rows="8" id="new_com">${message['data'][item].description}</textarea></br></br><input type="button" value="update" id="submit" onclick="updatecomment(${interventionId})"></span></item1>
	                    <item2><h1>${message['data'][item].title}</h1><blockquote>${message['data'][item].description} <a href="recorddet.html">more</a>&nbsp&nbsp&nbsp&nbsp&nbsp<a href="#" onclick=popText()>edit text</a></blockquote><p><span class="label">Latitude: ${message['data'][item].latitude}</span> &nbsp&nbsp&nbsp&nbsp&nbsp<span class="label">Longitude: ${message['data'][item].longitude}</span></p>
	                    ...
	                    </item2><p><span class="label">Status: ${message['data'][item].status}</span> </p>
	                        </form>
	                    </span></div>`
	                }
...
function updatelocation(interventionId){
    ...
    fetch(`https://ireporter-challenge4.herokuapp.com/api/v1/interventions/${interventionId}/location`,{
        method:'PATCH',
        headers: {
...
function del(interventionId){
    fetch(`https://ireporter-challenge4.herokuapp.com/api/v1/interventions/${interventionId}`,{
        method:'DELETE',
        headers:{
            'Content-type':'application/json',
            'x-access-token':valid_token
...
function updatecomment(interventionId){
    ...
    fetch(`https://ireporter-challenge4.herokuapp.com/api/v1/interventions/${interventionId}/comment`,{
        method:'PATCH',
        headers: {
            'Accept': 'application/json, text/plain, */*',
            'Content-type':'application/json',
        ...
        .then(function(message){
            ...
            if(message['data'][0]['message'] ==="updated intervention record's comment"){
                alert("updated intervention record's comment");
                window.location.replace('viewIntervention.html');
            }
